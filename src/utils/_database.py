...
def get_jdbc_options(config: Settings, database: str | None = None) -> dict[str, str]:
    ...
    if db.type == DatabaseType.MYSQL:
        ...
        options["url"] = f"jdbc:mysql://{db.host}:{db.port}/{db_name}?zeroDateTimeBehavior=convertToNull"
        ...
        options["url"] = f"jdbc:sqlserver://{db.host}:{db.port}{db_prop};encrypt=false;"
    ...
    if db.password:
        options["password"] = db.password.get_secret_value()
...
# --- Metadata Retrieval ---
def get_primary_keys(spark: SparkSession, config: Settings, table_name: str) -> dict[str, list[str]]:
    """Retrieves primary key information for the specified tables."""
    db_name = ""  # dummy
    db = config.database
    if db.type == DatabaseType.MYSQL:
        query = dedent(f"""
            SELECT TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, SEQ_IN_INDEX AS ORDINAL_POSITION
            FROM information_schema.STATISTICS
            WHERE CONCAT_WS('.', TABLE_SCHEMA, TABLE_NAME) IN ({table_name})
