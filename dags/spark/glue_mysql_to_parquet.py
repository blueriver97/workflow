...
def generate_lineage(config) -> tuple[list[Dataset], list[Dataset]]:
    ...
    for table in tables:
        ...
        all_inlets.append(Dataset(platform="mysql", name=f"{table}", env="PROD"))
        all_outlets.append(
            Dataset(platform="s3", name=f"s3://{bucket}{parquet_path}/{schema}/{table_name}", env="PROD")
        )
...
with DAG(
    ...
    description="export data from mysql to parquet on S3",
    schedule=None,
    ...
    export_tables = SparkSubmitOperator.partial(
        task_id=f"{DAG_ID}.spark-submit",
        conn_id="spark_default",
        application="/opt/airflow/src/mysql_to_parquet.py",
