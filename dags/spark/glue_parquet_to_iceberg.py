...
def get_mapped_configs(config):
    """expand_kwargs에 직접 전달될 '리스트'만 반환"""
...
def generate_lineage(config) -> tuple[list[Dataset], list[Dataset]]:
    ...
    bucket = Variable.get("AWS_S3_BUCKET", "bucket")
    parquet_path = Variable.get("AWS_PARQUET_PATH", "/data/raw")
    tables = config["job"]["tables"]
    ...
    for table in tables:
        parts = table.split(".")
        if len(parts) == 3:
            schema, _, table_name = parts
        else:
            schema, table_name = parts

        all_inlets.append(Dataset(platform="s3", name=f"s3://{bucket}{parquet_path}/{schema}/{table_name}", env="PROD"))
        all_outlets.append(
...
def generate_env(config) -> dict:
    ...
    database_type = job_config["database_type"]
    vault_secret_path = job_config["vault_secret_path"]
    ...
    application_env = {
        ...
        "VAULT__SECRET_PATH": vault_secret_path,
        "DATABASE__TYPE": database_type,
        ...
        "AWS__PARQUET_PATH": "{{ var.value.AWS_PARQUET_PATH }}",
    }
...
with DAG(
    ...
    description="convert S3 parquet files to iceberg tables",
    schedule=None,
    ...
    convert_tables = SparkSubmitOperator.partial(
        task_id=f"{DAG_ID}.spark-submit",
        conn_id="spark_default",
        application="/opt/airflow/src/parquet_to_iceberg.py",
