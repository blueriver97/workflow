...
def generate_env() -> dict:
    ...
    application_env = {
        ...
        "POLARIS__URI": "{{ var.value.POLARIS_URI }}",
        "POLARIS__OAUTH2_SERVER_URI": "{{ var.value.POLARIS_OAUTH2_SERVER_URI }}",
        "POLARIS__CREDENTIAL": "{{ var.value.POLARIS_CREDENTIAL }}",
        "POLARIS__WAREHOUSE": "{{ var.value.POLARIS_WAREHOUSE }}",
...
with DAG(
    ...
    description="Kafka → Iceberg streaming ingestion via Polaris catalog with graceful shutdown",
    schedule=None,
    ...
    tags=["spark", "streaming", "kafka", "iceberg", "polaris"],
) as dag:
    ...
    catalog = Variable.get("POLARIS_CATALOG", "polaris")
    ...
    spark_conf = {
        ...
        f"spark.sql.catalog.{catalog}.uri": "{{ var.value.POLARIS_URI }}",
        f"spark.sql.catalog.{catalog}.warehouse": "{{ var.value.POLARIS_WAREHOUSE }}",
        f"spark.sql.catalog.{catalog}.oauth2-server-uri": "{{ var.value.POLARIS_OAUTH2_SERVER_URI }}",
        f"spark.sql.catalog.{catalog}.credential": "{{ var.value.POLARIS_CREDENTIAL }}",
        f"spark.sql.catalog.{catalog}.scope": "PRINCIPAL_ROLE:ALL",
        f"spark.sql.catalog.{catalog}.header.Polaris-Realm": "default",
        f"spark.sql.catalog.{catalog}.header.X-Iceberg-Access-Delegation": "vended-credentials",
        f"spark.sql.catalog.{catalog}.token-refresh-enabled": "true",
