...
def generate_lineage(config) -> tuple[list[Dataset], list[Dataset]]:
    ...
    database_type = config["job"]["database_type"]
    tables = config["job"]["tables"]
    ...
    for table in tables:
        ...
        all_inlets.append(Dataset(platform=database_type, name=f"{table}", env="PROD"))
        all_outlets.append(
...
with DAG(
    ...
    description="validate source DB schema against Iceberg and sync comments",
    schedule="@daily",
    start_date=datetime(2026, 1, 1),
    catchup=False,
    on_failure_callback=on_failure,
    on_success_callback=on_success,
    tags=["spark", "schema", "validation"],
) as dag:
    ...
    validate_tables = SparkSubmitOperator.partial(
        task_id=f"{DAG_ID}.spark-submit",
        conn_id="spark_default",
        application="/opt/airflow/src/schema_validate.py",
